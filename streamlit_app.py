                    ...
                    for obj_name in xObject:
                        ...
                        if '/Subtype' in obj and obj['/Subtype'] == '/Image':
                            ...
                            try:
                                ...
                                # ULTRA-AGGRESSIVE MAP FILTERING
                                # Most property photos are PORTRAIT or NEAR-SQUARE
                                # Maps can be any aspect ratio but are often high-resolution and square/wide
                                ...
                                # FILTER 2: VERY STRICT aspect ratio - Prefer portrait to square images
                                # Most property photos: 0.67:1 (2:3 portrait) to 1.4:1 (slight landscape)
                                # Reject wide landscape (maps often wide) and extreme portrait
                                if aspect_ratio > 1.4 or aspect_ratio < 0.6:
                                    skipped_images += 1
                                    continue
                                
                                # FILTER 3: Maximum dimensions - maps are often very high resolution
                                # Skip images larger than 1800x1800 (typical property photos are smaller)
                                if width > 1800 or height > 1800:
                                    skipped_images += 1
                                    continue
                                
                                # FILTER 4: Skip perfect squares that are large (often maps)
                                # Property photos are rarely perfectly square
                                if abs(aspect_ratio - 1.0) < 0.05 and width > 1000:
                                    skipped_images += 1
                                    continue
...
def save_analysis_results(file_name, image_name, model_name, page_number, analysis_json, full_text):
    """
    Save analysis results to Snowflake table using Snowpark DataFrame API
    ...
    try:
        ...
        # Convert analysis_json to JSON string for METADATA
        metadata_json_str = json.dumps(analysis_json)
        
        # Create DataFrame with ONLY the columns we're providing values for
        # DO NOT include ID (auto-increment) or ANALYSIS_TIMESTAMP (default) - Snowflake handles these
        data = [(
            file_name,
            image_name,
            model_name,
            page_number,
            bool(for_sale.get('detected', False)),
            float(for_sale.get('confidence', 0)),
            bool(solar.get('detected', False)),
            float(solar.get('confidence', 0)),
            bool(human.get('detected', False)),
            float(human.get('confidence', 0)),
            bool(damage.get('detected', False)),
        ...
        # Convert METADATA column from string to VARIANT using parse_json
        df = df.with_column("METADATA", parse_json(df["METADATA"]))
        
        # Select only the columns we're inserting (exclude ID and ANALYSIS_TIMESTAMP)
        # Write to table - Snowflake will auto-generate ID and set ANALYSIS_TIMESTAMP
        df.write.mode("append").save_as_table(
            f"{DATABASE}.{SCHEMA}.{ANALYSIS_TABLE}",
